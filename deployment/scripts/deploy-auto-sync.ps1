...
ssh root@iranianchurchdc.com "mkdir -p /root/Mychurch/backend/migrations"

# Step 2: Upload migration SQL
Write-Host "`n📤 Uploading database migration..." -ForegroundColor Yellow
scp backend\migrations\create_sync_jobs_table.sql root@iranianchurchdc.com:/root/Mychurch/backend/migrations/

# Step 3: Run migration
Write-Host "`n🗄️ Running database migration..." -ForegroundColor Yellow
ssh root@iranianchurchdc.com "psql -U myuser -d mychurch -f /root/Mychurch/backend/migrations/create_sync_jobs_table.sql"

# Step 4: Upload syncWorker service
Write-Host "`n📤 Uploading Background Worker..." -ForegroundColor Yellow
ssh root@iranianchurchdc.com "mkdir -p /root/Mychurch/backend/services"
scp backend\services\syncWorker.js root@iranianchurchdc.com:/root/Mychurch/backend/services/

# Step 5: Upload updated routes
Write-Host "`n📤 Uploading updated worship routes..." -ForegroundColor Yellow
scp backend\routes\worshipRoutes.js root@iranianchurchdc.com:/root/Mychurch/backend/routes/

# Step 6: Upload updated server.js
Write-Host "`n📤 Uploading updated server..." -ForegroundColor Yellow
scp backend\server.js root@iranianchurchdc.com:/root/Mychurch/backend/

# Step 7: Restart backend
Write-Host "`n🔄 Restarting backend server..." -ForegroundColor Yellow
ssh root@iranianchurchdc.com "pm2 restart mychurch-backend"

Start-Sleep -Seconds 3

# Step 8: Check backend status
Write-Host "`n✅ Checking backend status..." -ForegroundColor Yellow
ssh root@iranianchurchdc.com "pm2 status mychurch-backend"

# Step 9: Show recent logs
Write-Host "`n📋 Recent backend logs:" -ForegroundColor Yellow
ssh root@iranianchurchdc.com "pm2 logs mychurch-backend --lines 30 --nostream"
...
ssh root@iranianchurchdc.com "rm -f /var/www/html/assets/index-*.js 2>/dev/null || true"
...
if ($bundleFile) {
    ...
    scp $bundleFile.FullName root@iranianchurchdc.com:/var/www/html/assets/
}

# Upload index.html
Write-Host "Uploading index.html..." -ForegroundColor Gray
scp dist\index.html root@iranianchurchdc.com:/var/www/html/
...
ssh root@iranianchurchdc.com "psql -U myuser -d mychurch -c '\d sync_jobs' 2>/dev/null | head -n 10"

Write-Host "`nChecking worker file..." -ForegroundColor Gray
ssh root@iranianchurchdc.com "ls -lh /root/Mychurch/backend/services/syncWorker.js"
...
Write-Host "🌐 Access: https://iranianchurchdc.com/#/admin/sync-management" -ForegroundColor Cyan
