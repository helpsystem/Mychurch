...
async function importSpanishBible() {
    ...
    try {
        ...
        // Read the Spanish Bible structure
        const structurePath = path.join(__dirname, 'bible-structure-spanish.json');
        ...
        for (const book of bibleData.books) {
            ...
            const bookData = {
                code: book.usfm,
                name_en: book.text || book.usfm, // English name if available
                name_fa: book.title, // Spanish name (stored in name_fa field)
        ...
        console.error('❌ Error importing Spanish Bible:', error);
        throw error;
    } finally {
        client.release();
...
if (require.main === module) {
    importSpanishBible().catch(console.error);
}

module.exports = { importSpanishBible };