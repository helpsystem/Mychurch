...
async function importPersianBible() {
    const client = await pool.connect();
    
    try {
        console.log('✅ Connected to database');
        
        // Read the Persian Bible structure
        const structurePath = path.join(__dirname, 'bible-structure-persian.json');
        ...
        for (const book of bibleData.books) {
            ...
            const bookData = {
                code: book.usfm,
                name_en: book.title_long || book.title || book.usfm, // English name fallback
                name_fa: book.title, // Persian name
                testament: book.canon === 'ot' ? 'OT' : 'NT', // Determine from canon or position
        ...
        console.error('❌ Error importing Persian Bible:', error);
        throw error;
    } finally {
        client.release();
...
if (require.main === module) {
    importPersianBible().catch(console.error);
}

module.exports = { importPersianBible };