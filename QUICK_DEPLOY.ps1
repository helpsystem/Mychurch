...
Write-Host ""
$deployCmd = 'cd /var/www/mychurch && git pull origin main && npm install && npm run build && cd backend && npm install --production && pm2 restart mychurch-backend && cd .. && pm2 status'
Write-Host $deployCmd -ForegroundColor Green
Write-Host ""
