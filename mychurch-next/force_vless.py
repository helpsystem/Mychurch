...
# 2. Build exact JSON settings Xray requires
settings = {
    "clients": [
        {
            "id": client_id,
...
# Wipe old if exists
c.execute("DELETE FROM inbounds WHERE remark='Iran_VLESS'")

c.execute('''
    INSERT INTO inbounds 
    (user_id, up, down, total, remark, enable, expiry_time, listen, port, protocol, settings, stream_settings, tag, sniffer) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
''', (1, 0, 0, 0, 'Iran_VLESS', 1, 0, '', 8443, 'vless', json.dumps(settings), json.dumps(stream_settings), 'inbound-8443', json.dumps(sniffer)))

conn.commit()
conn.close()

# 4. output final link
link = f"vless://{client_id}@195.250.25.185:8443?type=tcp&security=reality&sni=yahoo.com&fp=chrome&pbk={public_key}&sid={short_id}&flow=xtls-rprx-vision#Iran_VLESS"
