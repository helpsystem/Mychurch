...
$deployCmd = "set +e; if [ ! -f /swapfile ]; then echo 'Creating 2G swap...'; fallocate -l 2G /swapfile; chmod 600 /swapfile; mkswap /swapfile; swapon /swapfile; echo '/swapfile none swap sw 0 0' >> /etc/fstab; echo 'Swap created'; else swapon /swapfile 2>/dev/null; echo 'Swap active'; fi; set -e; cd ${VPS_NEXT_PATH}; rm -rf .next.old; if [ -d .next ]; then mv .next .next.old; fi; tar -xzf ${TAR_FILE} 2>/dev/null; rm -f ${TAR_FILE}; rm -rf .next.old; pm2 stop mychurch-next 2>/dev/null || true; if [ ! -f node_modules/.bin/next ]; then echo 'Installing deps...'; rm -rf node_modules || true; npm install --omit=dev --legacy-peer-deps; echo 'npm install done'; fi; if pm2 show mychurch-next > /dev/null 2>&1; then pm2 restart mychurch-next --update-env; else pm2 start npm --name mychurch-next -- start; fi; pm2 save; if [ -d /var/cache/nginx ]; then rm -rf /var/cache/nginx/*; fi; nginx -s reload; echo DONE"
