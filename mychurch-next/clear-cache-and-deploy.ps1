...
$deployCmd = "set +e; if [ ! -f /swapfile ]; then echo 'Creating 2G swap...'; fallocate -l 2G /swapfile; chmod 600 /swapfile; mkswap /swapfile; swapon /swapfile; echo '/swapfile none swap sw 0 0' >> /etc/fstab; echo 'Swap created'; else swapon /swapfile 2>/dev/null; echo 'Swap active'; fi; set -e; cd ${VPS_NEXT_PATH}; rm -rf .next.old; if [ -d .next ]; then mv .next .next.old; fi; tar -xzf ${TAR_FILE} 2>/dev/null; rm -f ${TAR_FILE}; rm -rf .next.old; pm2 delete mychurch-next 2>/dev/null || true; pm2 delete socket-server 2>/dev/null || true; set +e; echo 'Installing deps...'; rm -f ../package-lock.json ../package.json 2>/dev/null || true; npm install --legacy-peer-deps; echo 'npm install done'; pm2 start node_modules/next/dist/bin/next --name mychurch-next -- start; pm2 start socket-server.js --name socket-server; pm2 save; if [ -d /var/cache/nginx ]; then rm -rf /var/cache/nginx/*; fi; systemctl reload nginx 2>/dev/null || nginx -s reload; echo 'Waiting for app to start...'; sleep 3; pm2 logs mychurch-next --lines 50 --nostream; echo DONE"
