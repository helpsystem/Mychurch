...
import PrayerListModal from "./PrayerListModal";
...
export default function LiveConsole({ initialPresentationId = null }: LiveConsoleProps) {
    ...
    const [isPrayerListOpen, setIsPrayerListOpen] = useState(false);
        ...
        >
            ...
            <header className="h-16 px-3 sm:px-5 border-b border-border/10 flex items-center justify-between bg-neutral-900/95 backdrop-blur-md shrink-0 z-20 w-full shadow-md gap-2">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
                    ...
                    {/* Prayer List Button */}
                    <button
                        onClick={() => setIsPrayerListOpen(true)}
                        className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 rounded-lg font-bold text-xs sm:text-sm transition-all shadow-md cursor-pointer border bg-rose-600/20 text-rose-400 border-rose-500/30 hover:bg-rose-600/30"
                        title={isRTL ? "مدیریت لیست دعا و ارسال به تلگرام" : "Manage Prayer List & Send to Telegram"}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-400">
                            <path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" />
                        </svg>
                        <span className="hidden sm:inline">{isRTL ? "لیست دعا" : "Prayer List"}</span>
                    </button>
            ...
            {/* Prayer List Modal */}
            <PrayerListModal
                isOpen={isPrayerListOpen}
                onClose={() => setIsPrayerListOpen(false)}
                isRTL={isRTL}
