...
export default function ChurchDocumentsPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("mychurch_custom_gemini_key");
  ...
  const handleSaveCustomKey = (key: string) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("mychurch_custom_gemini_key", key);
    }
  };
  ...
  const callAI = useCallback(async (mode: string, body?: string) => {
    const savedKey = typeof window !== "undefined" ? localStorage.getItem("mychurch_custom_gemini_key") || "" : "";
    const res = await fetch("/api/ai/letters", {
      ...
      body: JSON.stringify({ 
        ...
        lang: editLang, 
        topic: aiTopic, 
        body: body || (editLang === "en" ? bodyEn : bodyFa),
        userApiKey: savedKey 
      }),
    });
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                {settingsTab === "info" ? (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4 p-4 rounded-2xl bg-purple-500/[0.02] border border-purple-500/10">
                       <h3 className="text-xs font-black uppercase text-purple-400 flex items-center gap-1.5">
                         <Sparkles className="w-3.5 h-3.5 text-purple-400" /> {isRtl ? "تنظیمات هوش مصنوعی (کلید اختصاصی)" : "AI Settings (Custom Key)"}
                       </h3>
                       <div>
                         <label className="text-[10px] text-muted-foreground mb-1.5 block leading-relaxed">
                           {isRtl 
                             ? "در صورت اتمام سهمیه عمومی، کلید اختصاصی API جمینای خود را از Google AI Studio گرفته و در اینجا وارد کنید (کاملا رایگان):"
                             : "If you exceed the shared API quota, get your own free Gemini API key from Google AI Studio and enter it below:"}
                         </label>
                         <input
                           type="password"
                           ...
                           placeholder="AIzaSy..."
                           className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-xs focus:outline-none focus:border-purple-500/50 transition-colors"
                         />
                         <p className="text-[9px] text-slate-500 mt-1.5 flex items-center gap-1 leading-normal flex-wrap">
                           <span>{isRtl ? "کلید شما در مرورگر ذخیره شده و مستقیماً برای سرور ارسال می‌شود." : "Your key is safely stored locally in your browser and sent securely to the API."}</span>
                           <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-purple-400 font-bold hover:underline inline-flex items-center gap-0.5">
                             {isRtl ? "دریافت کلید رایگان" : "Get Free Key"} <ExternalLink className="w-2.5 h-2.5" />
                           </a>
                         </p>
                       </div>
                    </div>
