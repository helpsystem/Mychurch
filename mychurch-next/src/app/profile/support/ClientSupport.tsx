...
const localDict = {
    en: {
        ...
        pageSubtitle: "Submit feature requests, report issues, or contact church leaders.",
        newTicket: "New Ticket",
        back: "Back",
        newTicketFormTitle: "Submit a New Ticket",
        ...
        ticketClosed: "Closed",
        ticketPrefix: "Ticket #",
        mockUserMessage: "Hello, I need help with... This is a sample message shown to demonstrate the chat UI.",
        adminReply: "Admin Reply",
        mockAdminMessage: "Hello, thank you for your message. We are looking into it.",
        typeMessage: "Type a message...",
        noTicketSelected: "Select a ticket to view the conversation",
        toastCreated: "Support ticket created.",
        toastError: "Failed to create ticket.",
        locale: "en-US",
    },
    fa: {
        pageTitle: "پشتیبانی و پیام‌ها",
        pageSubtitle: "Submit feature requests, report issues, or contact church leaders.",
        newTicket: "تیکت جدید",
        back: "بازگشت",
        newTicketFormTitle: "ثبت تیکت جدید",
        subjectLabel: "موضوع",
        messageLabel: "پیام شما",
        submitTicket: "ارسال تیکت",
        ticketListTitle: "لیست تیکت‌ها",
        noTicketsFound: "هیچ تیکتی یافت نشد.",
        statusOpen: "Open",
        statusClosed: "Closed",
        ticketClosed: "بسته شده",
        ticketPrefix: "تیکت #",
        mockUserMessage: "سلام، من نیاز به کمک در مورد... دارم. این یک پیام آزمایشی برای نمایش UI چت است.",
        adminReply: "پاسخ ادمین",
        mockAdminMessage: "سلام، با تشکر از پیام شما. بررسی می‌شود.",
        typeMessage: "تایپ پیام...",
        noTicketSelected: "یک تیکت را برای مشاهده گفتگو انتخاب کنید",
        toastCreated: "تیکت پشتیبانی ایجاد شد.",
        toastError: "خطا در ایجاد تیکت",
        locale: "fa-IR",
    },
    es: {
        pageTitle: "Soporte y Mensajes",
        pageSubtitle: "Envíe solicitudes de funciones, reporte problemas o contacte a los líderes de la iglesia.",
        newTicket: "Nuevo Ticket",
        back: "Atrás",
        newTicketFormTitle: "Enviar un Nuevo Ticket",
        subjectLabel: "Asunto",
        ...
        ticketPrefix: "Ticket #",
        mockUserMessage: "Hola, necesito ayuda con... Este es un mensaje de ejemplo para mostrar la interfaz del chat.",
        adminReply: "Respuesta del Administrador",
        mockAdminMessage: "Hola, gracias por su mensaje. Lo estamos revisando.",
        typeMessage: "Escriba un mensaje...",
        noTicketSelected: "Seleccione un ticket para ver la conversación",
        toastCreated: "Ticket de soporte creado.",
...
export default function ClientSupport({ initialTickets, userEmail }: { initialTickets: SupportTicket[], userEmail: string }) {
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 font-[Vazirmatn]" dir={isRTL ? "rtl" : "ltr"}>
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-slate-400 mt-2">{d.pageSubtitle}</p>
                </div>
                <button
                ...
                >
                    <Plus className="w-5 h-5" /> {d.newTicket}
                </button>
            </div>
            ...
            {isCreating ? (
                <div className="bg-zinc-900 border border-white/10 rounded-3xl p-8 shadow-2xl relative">
                    <button onClick={() => setIsCreating(false)} className="absolute top-6 left-6 text-slate-400 hover:text-white flex items-center gap-2 text-sm font-bold">
                        {d.back} <ArrowRight className="w-4 h-4" />
                    </button>

                    <h2 className="text-xl font-black text-white mb-6">{d.newTicketFormTitle}</h2>
                    <form onSubmit={handleCreate} className="space-y-5 max-w-2xl">
                        <div>
                            <label className="text-xs font-bold text-slate-400 mb-2 block">{d.subjectLabel}</label>
                            <input
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-400 mb-2 block">{d.messageLabel}</label>
                            <textarea
                        ...
                        <button disabled={isPending} type="submit" className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 disabled:opacity-50">
                            {isPending ? <Loader2 className="w-5 h-5 animate-spin"/> : <Send className="w-5 h-5"/>} {d.submitTicket}
                        </button>
                    </form>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[600px]">
                    ...
                    <div className="col-span-1 bg-zinc-950 border border-white/10 rounded-3xl overflow-y-auto flex flex-col">
                        <div className="p-4 border-b border-white/10 bg-zinc-900/50 flex-shrink-0 sticky top-0 z-10">
                            <h3 className="font-black text-white px-2">{d.ticketListTitle}</h3>
                        </div>
                            ...
                            {tickets.map(t => (
                                ...
                                >
                                    <div className="flex justify-between items-start w-full">
                                        ...
                                        {t.status === 'open' && <span className="bg-emerald-500/20 text-emerald-400 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider">{d.statusOpen}</span>}
                                        {t.status === 'closed' && <span className="bg-slate-800 text-slate-400 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider">{d.statusClosed}</span>}
                                    </div>
                                    <div className="text-xs text-slate-500 flex items-center gap-1">
                                        <Clock className="w-3 h-3" /> {new Date(t.created_at).toLocaleDateString(d.locale)}
                                    </div>
                                </button>
                            ))}
                            {tickets.length === 0 && (
                                <div className="p-8 text-center text-slate-500 text-sm font-medium">{d.noTicketsFound}</div>
                            )}
                        </div>
                    </div>
                            ...
                            <>
                                <div className="p-6 border-b border-white/10 bg-zinc-950/50 flex justify-between items-center">
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-400 mt-1">{d.ticketPrefix}{activeTicket.id.split('-')[0]}</div>
                                    </div>
                                    {activeTicket.status === 'closed' && (
                                        <div className="flex items-center gap-1.5 text-emerald-500 bg-emerald-500/10 px-3 py-1.5 rounded-lg text-sm font-bold">
                                            <CheckCircle2 className="w-4 h-4"/> {d.ticketClosed}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex-1 p-6 overflow-y-auto space-y-6 flex flex-col">
                                    ...
                                    <div className="flex gap-4 max-w-[80%] self-start">
                                        ...
                                        <div>
                                            <span className="text-xs text-slate-400 font-bold mb-1 block">{d.adminReply}</span>
